...
def index(request):
    ...
    check_user = request.session.get('user', 'Cmn001')
    users = User.objects.filter(is_staff=False)
    available_model(request)
    # model = Obj1Ai
    model = available_model(request, Cmn=False)
    # print(model)
...
# second page with detail information
@login_required(login_url='/authenticate/')
def detail(request, ai):
    if request.POST:
        if bool(request.POST.get('logout', False)):
            logout(request)
            return redirect('/authenticate/')
        if request.user.is_staff and request.POST.get('user', 'Cmn001'):
            request.session['user'] = request.POST.get('user')
    ...
    errors = (count_error(ai, model) for ai in sensors)
    sensors_errors = list(zip(sensors, errors))
    return render(request, 'tables/detail.html', {'now': now, 'filters': filters, 'sensors_errors': sensors_errors, 'check_user': check_user, 'users': users})
...
def archive(request):
    ...
    errors = (count_error(ai, model) for ai in sensors)
    sensors_errors = list(zip(sensors, errors))
    return render(request, 'tables/archive.html', {'now': now, 'sensors_errors': sensors_errors, 'check_user': check_user, 'users': users})
